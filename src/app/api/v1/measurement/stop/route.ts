...
export async function POST() {
  try {
    ...
    const backendUrl = buildBackendApiUrl("measurement/stop");
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: "Gagal menghentikan pengukuran" },
    ...
    console.error("Stop measurement proxy error:", error);
    return NextResponse.json(
