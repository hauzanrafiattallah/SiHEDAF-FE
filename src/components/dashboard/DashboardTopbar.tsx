...
import {
  ProfileAvatar,
  profileInitials,
} from "@/components/dashboard/ProfileAvatar";
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center justify-between gap-4">
      ...
      <div className="flex shrink-0 items-center gap-4">
        ...
        <Link
          aria-label={`Buka profil ${fullname}`}
        ...
        >
          {user ? (
            <ProfileAvatar
              className="h-7 w-7 border-0 text-[11px]"
              fullname={user.fullname}
              profileImage={user.profileImage}
            />
          ) : (
            <span className="grid h-7 w-7 place-items-center rounded-full bg-[linear-gradient(135deg,#d7ebff,#8eb9e7)] text-[11px] font-semibold text-primary-900">
